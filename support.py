...
CSS = '''
div.easy-diff { padding: 10px; margin: 0; }
.easy-diff h1, .easy-diff h2, .easy-diff h3,
.easy-diff h4, .easy-diff h5, .easy-diff h6 {
    {{'.string'|css}}
}
.easy-diff blockquote { {{'.comment'|css}} }
...
class EasyDiffSupportInfoCommand(sublime_plugin.ApplicationCommand):
    """Support info."""
...
class EasyDiffOpenSiteCommand(sublime_plugin.ApplicationCommand):
    """Open site links."""
...
class EasyDiffDocCommand(sublime_plugin.WindowCommand):
    """Open doc page."""
    ...
    def run(self, page):
        ...
        if not has_phantom_support:
            ...
            view.set_name('EasyDiff - Quick Start')
...
class EasyDiffChangesCommand(sublime_plugin.WindowCommand):
    """Changelog command."""
    ...
    def run(self):
        ...
        view = self.window.new_file()
        view.set_name('EasyDiff - Changelog')
        view.settings().set('gutter', False)
        view.settings().set('word_wrap', False)
        if has_phantom_support:
            mdpopups.add_phantom(
